...
server.post("/upliftments/", function(req, res, next) {
  const error = validateUpliftment(req.body);
  if (error) {
...
function validateUpliftment(upliftment) {
  if (!upliftment.title) return "Title is required.";
  if (!upliftment.clientId) return "Client is required.";
  if (!upliftment.category) return "Category is required.";
  return "";
}
